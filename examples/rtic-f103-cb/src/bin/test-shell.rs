...
mod app {
  ...
  use foc_simple::foc::foc_rtic::{FocRtic, FocSimpleCommand};
  use foc_simple::foc::foc_rtic::{SIGNAL_10MS, SIGNAL_1MS};
  ...
  use rtic_f103_cb::foc_impl::{pwm_driver::PwmDriverImpl, FocSerialImpl, SensorAngleDummy, SensorCurrentImpl};
  ...
  fn init(mut cx: init::Context) -> (Shared, Local) {
    ...
    let mut foc_rtic = FocRtic::new(0, foc_simple, foc_pwm);
    foc_rtic.set_angle_sensor(EFocAngleSensor::HallSensor);
  ...
  async fn command_task(context: command_task::Context) {
    context.local.command.task().await;
  }

  // ticker task generating signals with 1000 hrz and 100 hrz
  #[task(binds=TIM2, local=[timer1ms, counter], priority = 3)]
  fn ticker_task(cx: ticker_task::Context) {
    ...
    SIGNAL_1MS[0].signal(());
    cx.local.timer1ms.clear_interrupt(Event::Update);
  }

  // motor task, waiting for incoming signals for 1 ms, 10 ms, hall sensor and commands
  #[task(local = [foc_rtic], priority = 2)]
  async fn motor_task(mut cx: motor_task::Context) {
    cx.local.foc_rtic.task().await;
  }
  /// note that  C13 (push button on nucleo board) will also generate this interrupt
  #[task(binds=EXTI15_10,  local=[hall1, hall2, hall3, hall_sensor], priority = 2)]
  fn hall_sensor_task(mut context: hall_sensor_task::Context) {
    ...
    match context.local.hall_sensor.update(phase) {
      Ok(angle) => HALL_ANGLES[0].signal(angle),
